...
def pot_parking_spot(orig_img,inf_img,points_2d_ls,save_results,display):
    '''
    ...
    if(flag_top == 1 and flag_bottom == 1):
        if len(str(save_results))!=0 or display==1:
...
class carla_utils():
    ...
    def __init__(self):
        ...
        self.display=int(self.config_object["Display_results"]["Display"])
    ...
    def run_seg(self,frame,world_points):
        ...
        #self.rect_pts
        res_img,self.rect_pts = pot_parking_spot(frame,self.seg_img,self.points_2d_ls,self.save_results,self.display)
